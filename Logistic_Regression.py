...
featuresX = pd.DataFrame(data[columnsForFeatures].values, columns = columnsForFeatures)
targetY = pd.Series(data['Survived'])
...
log_model.fit(featuresX_train, targetY_train)
targetY_prediction = log_model.predict(featuresX_test)

# Calculate accuracy
log_model_accuracy = accuracy_score(targetY_test, targetY_prediction)
print(f"Model Accuracy: {log_model_accuracy}")
